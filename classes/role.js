...
class Role {
    ...
    async insert(){
        const results = await RoleModel.insert(this._Role);
        this._Role.id = results.insertId;
    }

    async getAll(){
        this._Role = await RoleModel.selectAll();
    }

    async getById(id){
        this._Role = await RoleModel.selectByID(id);
    }
    ...
    async update(id){
        await RoleModel.update(id, this._Role);
    }
